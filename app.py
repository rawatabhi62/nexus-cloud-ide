...
def join_room_page(room_id):
    if room_id not in rooms:
        # Auto-create if expired or missing
        rooms[room_id] = {
            'code': "# Welcome to Nexus Global Cloud IDE\nname = input('Enter your name: ')\nprint(f'Hello, {name}!')",
...
def save_code():
    ...
    if room_id in rooms:
        ...
        rooms[room_id]['activity_log'].insert(0, f"💾 {username} updated code checkpoint.")
...
def ai_chat():
    ...
    reply = ""
    if 'explain' in query:
        reply = f"🤖 AI Copilot: Your script has {len(code.splitlines())} lines. It is structured for optimal cloud execution and secure sandboxing."
    elif 'optimize' in query:
        reply = "🤖 AI Copilot Tip: Avoid deep nesting and use built-in functions to maximize runtime speed."
    elif 'bug' in query or 'error' in query:
        reply = "🤖 AI Audit: No syntax faults or critical security risks detected in the active scope."
    else:
        reply = f"🤖 AI Copilot: I analyzed your request. You can ask me to 'explain code', 'optimize', or 'find bugs'."
...
def execute_code():
    ...
    try:
        ...
        output = "❌ Execution Error: Process timed out (Possible infinite loop or waiting for input)."
...
<body style="background:#090d16; color:#fff; font-family:'Segoe UI',sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; margin:0;">
    <div style="text-align:center; background:#111827; padding:45px; border-radius:12px; border:1px solid #1f2937;">
        ...
        <p style="color:#94a3b8; margin-bottom:25px;">Professional Collaborative Cloud IDE Engine.</p>
    ...
    <div class="main-content">
        <div class="header">
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
                <input type="text" id="stdinInput" placeholder="Stdin input (if input() used)..." style="width: 200px; font-size: 12px;">
        ...
        <div class="terminal-pane">
            ...
            <pre id="outputBox">Console ready... Type any required input in the top input bar and click 'Execute Code'.</pre>
        </div>
    </div>
    <script>
        ...
        function saveCloud() {
            fetch('/api/save_code', {
                ...
                alert('✅ Code successfully saved to cloud checkpoint!');
            });
        }
