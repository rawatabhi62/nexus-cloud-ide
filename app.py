...
def handle_ai_chat(data):
    ...
    reply = ""
    if 'explain' in query:
        reply = f"🤖 AI Copilot: Script has {len(code.splitlines())} lines. Built with real-time WebSockets & secure sandboxing."
    elif 'optimize' in query:
        reply = "🤖 AI Copilot: Use built-in functions and list comprehensions to boost execution speed."
    elif 'bug' in query or 'error' in query:
        reply = "🤖 AI Audit: No syntax or recursion locks detected."
    else:
...
def handle_exec(data):
    ...
    try:
        ...
        output = "❌ Execution Error: Process timed out (Infinite loop or waiting for missing input)."
    except Exception as e:
        output = f"Execution Error: {str(e)}"
    
    if not output: output = "[Executed with no output]"
    rooms[room_id]['activity_log'].insert(0, f"▶ {username} executed {lang.upper()}.")
...
<style>
    ...
    pre { margin: 0; padding: 12px; font-size: 13px; color: #4ade80; overflow-y: auto; flex: 1; white-space: pre-wrap; background: #020617; border: none; outline: none; }
    ...
    <div class="main-content">
        ...
        <div class="terminal-pane">
            <div class="terminal-header">
                <span>📊 Output Console (Type input directly below when prompted)</span>
                <span style="color: #4ade80;">● Interactive Terminal Active</span>
            </div>
            <pre id="outputBox" contenteditable="true" spellcheck="false">Console ready... Click 'Execute Code' to run.</pre>
        </div>
    </div>
    <script>
        const socket = io();
        ...
        function runCode() {
            let lang = document.getElementById('langSelect').value;
            // Get text written inside output terminal as stdin input
            let terminalText = outputBox.innerText;
            let lines = terminalText.split('\\n');
            let userInput = lines.length > 0 ? lines[lines.length - 1].replace("Enter your name:", "").trim() : "";
